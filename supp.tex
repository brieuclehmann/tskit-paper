...
The practical consequence was demonstrated in a benchmark where using
the succinct tree sequence format and sequential iteration was more
...
simulation~\citep{kelleher2018efficient}. This publication also
...
corresponding reductions in statistic-computation time; and
\citet{lehmann2026on} introduced the operation of computing genetic
relatedness matrices (GRMs) directly from an ARG.
For one million diploids on human chromosome~21, the branch
GRM--vector product completes in under 18~seconds, and the associated
randomised PCA scales to $2^{20}$ samples where genotype-matrix PCA
runs out of memory.
\citet{lee2026genetic} apply this last operation to fitting linear
mixed models and provide the one recent head-to-head benchmark of
tskit against another library, comparing the ARG-based GRM with the
corresponding implementation in
ARGneedle-lib~\citep{zhang2023biobank}, and reporting that tskit is
one to two orders of magnitude faster for GRM--vector products across
a wide range of mutation rates, while using an exact algorithm that
avoids Monte-Carlo approximation.
...
Alongside these core additions, the exposure of tskit's traversal
orders to Numba~\citep{lam2015numba} has allowed Python-level user
code to JIT-compile ARG traversals at close to C speed, enabling
downstream packages such as
...
provides a documented low-level API intended for downstream reuse, and
...
arg-needle-lib version 1.2.1, BTE version 0.9.3, and DendroPy version
5.0.8.
